...
behavior/
  ...
  tests/        native Simulink Test component suite (7 suites, 21 cases)
```
...
The 21 behavioral component cases live in the native Simulink Test Manager file [`../behavior/tests/BehaviorComponentTests.mldatx`](../behavior/tests/BehaviorComponentTests.mldatx). Its seven suites correspond to `BehStorage`, `BehPrepUnit`, `BehCookLine`, `BehCookVat`, `BehQCStation`, `BehPackager`, and `BehSupervisor`. Every case is a baseline test, carries the `'behavior'` tag, simulates its component model directly with a finite stop time, and retains its input and output runs in the Test Manager result.

There are three run modes:

```matlab
runAllTests("behavior")  % component tier through the unified project runner
runBehTests()            % component tier, text pass/fail output
runBehTests("view")      % run, then open Test Manager on the results
```

Open Test Manager directly, or use `"view"`, to inspect each case's simulation output, custom-criteria assessments, and baseline-comparison plots. Every scenario compares its important output waveforms against a reviewed native baseline in [`../behavior/tests/baselines/`](../behavior/tests/baselines/) with signal-specific absolute, relative, leading, and lagging tolerances. Those 21 `.mat` files, the shared Dataset input file, and the `.mldatx` test definition are registered, source-controlled test artifacts. If a model change intentionally changes accepted behavior, inspect the failing traces and functional assertions first, then explicitly replace all references with `updateBehaviorBaselines(true)` and review the resulting baseline-file diff. Normal test execution never updates a baseline.

The deterministic [`../behavior/tests/buildBehaviorTestFile.m`](../behavior/tests/buildBehaviorTestFile.m) generator is the source of truth for case setup, stimuli, parameter overrides, tolerances, and custom criteria. Run it without arguments to rebuild against the committed baselines. The CookVat reference case additionally checks legal state transitions, one complete batch per Drain interval, flow only during Drain, total drained mass, temperature and power bounds, and nonnegative hopper level. All 21 cases pass in about one minute on the reference development machine.

The tests earned their keep as a cross-check on the models, not just on themselves. The Supervisor suite caught a real supervisor bug: a per-line enable vector sized for the wrong number of lines, which passed compilation (Simulink does not statically catch a mismatched-but-plausible vector width against a downstream mux) and only surfaced as a dimension mismatch once the test drove all lines through a fault transition. Two additional traps turned out to be on the *test* side rather than the model side, and cost real debugging time before being understood as API behavior rather than model defects — both are recorded in §8: model-workspace parameter override scope and interpolation of step stimuli.
...
- **Root outport Dataset elements are not reliably retrievable by name.** Their Dataset element names can be empty even when the root Outport blocks are named. The generated custom criteria therefore index `test.sltest_simout.get('yout')` in root-outport port order; the builder keeps those mappings next to each case definition.
