...
For behavioral-component work, `runAllTests("behavior")` or `runBehTests()` runs the 21 baseline cases in `behavior/tests/BehaviorComponentTests.mldatx`. Use `runBehTests("view")` to leave Test Manager open and inspect simulation outputs, assessments, and actual-versus-baseline comparisons. The 21 accepted native baseline `.mat` files under `behavior/tests/baselines/` are project-registered source artifacts. Replace them only after reviewing an intentional behavior change, using `updateBehaviorBaselines(true)`.
...
| `behavior/` | Behavioral component library (model references + subsystem references), data dictionaries, build scripts, and a native Simulink Test Manager component suite with tracked waveform baselines. The library is instantiated inline inside the three physical architecture models rather than composed in separate plant models. See [`docs/09_behavioral_models.md`](docs/09_behavioral_models.md) / [`docs/10_behavioral_trade_update.md`](docs/10_behavioral_trade_update.md). |
...
| `requirements/SystemRequirements.slreqx` | Requirements | 30 system requirements (SR-GS-001..028, with SR-GS-025 decomposed into SR-GS-025.1/.2), Derive-linked to stakeholder needs. |
...
The project compares three physical variants, `HyperCook`, `LeanBroth`, and `EverSimmer`, using a requirements-traceable trade study that combines roll-up metrics, formal compliance checks, and executable behavioral analysis. The current HyperCook roll-up is **HyperCook: 500.4 kW, 2097.2 kCr, 418.2 m^3**, and the formal gate reports **20/24 checks pass** across the three candidates. `EverSimmer` wins three of the four stakeholder-weighted scenarios and **85.5%** of the canonical Monte Carlo weight-sensitivity sweep — `LeanBroth` takes CostLean, though it fails the SR-GS-002 throughput floor. ADR-009 originally adopted `EverSimmer`, but ADR-035 reopened that decision. **No variant is committed as baseline** in the current artifacts: all three candidates keep their `Implement` links and their own test evidence, so each one's compliance story can be read on its own terms.

That matters because the three candidates fail in different places, and an any-variant status rollup hides it. `EverSimmer` verifies 10 system requirements by executed simulation but misses SR-GS-015 at 0.1 g; `HyperCook` verifies 9 including SR-GS-015, yet fails the power, cost and volume gates and SR-GS-026; `LeanBroth` verifies only 4 — startup readiness and two logistics requirements — falling short on throughput, rocket turnaround, and single-fault retention. Requirements Toolbox rolls status up over all loaded link sets and cannot scope to one candidate, so per-variant status is instead computed by link attribution — `tests/runAllTests` prints the per-candidate coverage table, and [`analysis/reporting/makeVariantTraceMatrix.m`](analysis/reporting/makeVariantTraceMatrix.m) generates the side-by-side design-review evidence in [`docs/deliverables/variantRequirementsTraces.md`](docs/deliverables/variantRequirementsTraces.md).
